...
async function main() {
  ...
  // Если новые категории уже есть — не перезаписываем
  const hasCar = await prisma.category.findUnique({ where: { slug: "car-accessories" } });
  if (hasCar) {
    console.log("Данные уже загружены. Пропуск.");
    return;
  }
